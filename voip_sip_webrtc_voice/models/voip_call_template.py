...
class VoipCallTemplateInheritVoice(models.Model):
    ...
    def _synth_bing(self, rendered_text, to_address, record_id):
        ...
        with open("/odoo/ms.wav", "wb") as out_file:
            ...
            output_filepath = tempfile.gettempdir() + "/output.raw"
            subprocess.call(['sox', "/odoo/ms.wav", "--rate", str(self.codec_id.sample_rate), "--channels", "1", "--encoding", self.codec_id.encoding, "--type","raw", output_filepath])
